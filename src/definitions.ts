...
export const TOOL_DEFINITIONS: ToolDefinition[] = [
  ...
  // ── Availability ───────────────────────────────────────────────
  {
    ...
    description: 'Check when a single agent is free across its Chronary calendars and any human calendars authorized for that agent. This tool is fail-closed: always inspect `availability_state` and `warnings` before using slots. Accepts `start`/`end` or the `start_time`/`end_time` aliases.',
  ...
  },
  {
    ...
    description: 'Find slots when multiple agents are free across Chronary calendars and any human calendars authorized for each agent. This tool is fail-closed: always inspect `availability_state` and `warnings` before using slots. Accepts `agents`/`start`/`end` or their aliases.',
  ...
  },
  {
    ...
    description: 'Poll a human-calendar setup request. Returns provider-neutral status and resulting connection ID when complete, but never replays the bearer setup URL or exposes human calendar data.',
  ...
  },
  {
    ...
    description: 'Cancel a pending human-calendar setup request. This invalidates the setup URL and does not disconnect an already completed Google or Microsoft connection.',
