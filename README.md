...
AI-agent framework adapters for [Chronary](https://chronary.ai) — 23 calendar tools usable directly by Vercel AI SDK, LangChain, Mastra, OpenAI function calling, and MCP clients.
...
Every adapter produces the same 23 calendar tools in the format the target framework expects. Tool definitions live in [`src/definitions.ts`](./src/definitions.ts); Zod schemas are in [`src/schemas.ts`](./src/schemas.ts).
